...
function TutorLearnerAssessmentLists(props) {
    ...
    const handleArchive = (assessment_id,assessment_learner_user_id) => {
        console.log('assessment_id',assessment_id);
        console.log('assessment_learner_user_id',assessment_learner_user_id);
        let formData = { 'assessment_learner_user_id': assessment_learner_user_id }
        CommonService.updatePost('learnersAssessmentArchive', assessment_id, formData)
        .then(response => {
                                ...
                                <tr key={assessment.id}>
                                    ...
                                    <td>
                                        {(function() {
                                            if (assessment.status==='result_out') { 
                                                return <div>
                                                        <Link to={"/tutor/learner-assessment-result/" + assessment.id +'/'+assessment.learner_user_id} className="detailsSection">View Result <i className="fa fa-angle-right"></i></Link> 
                                                        <button className="progressBtn" onClick={e=>handleArchive(assessment.id,assessment.learner_user_id)}>Archive</button>                                                
                                                    </div>
                                            } else if(assessment.status=='completed') {
