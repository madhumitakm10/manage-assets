...
// 
    ...
    const showFile = (id) => {
        ...
        setAudioLoader(true)
        setFileLoader(true)
        CommonService.getById('questionAnswer/addFileNew', id)
            .then(response => {
                ...
                if (response.data.success) {
                    ...
                    setShowMediafile(response?.data?.data?.fileData);
                ...
                .then(response => {
                    ...
                    if(response.data.message=='validation_failed'){
                        ...
                        if(response.data.success){
                            toast.success(response.data.message);
                            //props.history.push("/admin/question-answer");
                            props.history.push("/admin/question-answer/edit/"+response.data.data.id);
                        }
                    }                    
                })
                .catch(e => {
    ...
    const onUploadFile = (e, key) => {
        ...
        if (!!dataSend.image_name || !!dataSend.audio_name || !!dataSend.video_name || !!dataSend.file_name) {
            if (props?.match?.params?.id) {
                CommonService.updatePost('questionAnswer/addFileNew', props.match.params.id, dataSend)
                    .then(response => {
                    ...
                    <CustomLoader /> : 
                        <Form onSubmit={handleSubmit}>
                            <div className="">
                                ...
                                {showMediafile.map((media, index) => (

                                    <Form.Group className="col-md-6" key={media.id}>
                                        {(function() {
                                            if (media?.file_type=='image') {
                                                // return <img width="140" height="81"
                                                // src={question?.mediaFile?.file_url} alt="" />;
                                                return <img width="140" height="81"
                                                src={media?.file_url ? media?.file_url: noImg} alt="" />;
                                            } else if(media.file_type=='audio' || media.file_type == 'audio_record') {
                                                return <audio className="uploadBtnsImg h-40" src={media?.file_url} controls />;
                                            } else if(media.file_type=='video') {
                                                return <ReactPlayer controls width="500px" height="220px" url={media?.file_url} />;
                                            }
                                            else if(media.file_type=='file') {
                                                return <p className="pdfDownload" onClick={()=> window.open(media.file_url, "_blank")}><img width="140" height="81"
                                                src={pdfImg} alt="" /></p>;
                                            }
                                        })()}
                                ...
                                <div className="eachImgUpWrap videoDiv mt-4">
                                    ...
                                    <div className="d-flex eachImgUpBox">
                                        ...
                                        <div className="uploadedBtn">
                                            ...
                                            <button type="button" className="uploadBtnsImg ml-3 red_bg_btn" onClick={deleteFileData.bind(this,props?.match?.params?.id,'video')}> Delete </button>
                                        </div>
